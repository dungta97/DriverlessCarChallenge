...
void RunCar()
{
    ...
    writer.open("./colorImg.avi",CV_FOURCC('M','J','P','G'),25.0,Size(320,240),true);
    //writer2.open("./birdView "+to_string(videoidx)+".avi",CV_FOURCC('M','J','P','G'),25.0,Size(320,240),true);
    clearLCD();
...
double getAngleSteering()
{
	...
	// xet tung vach ke, do rong moi vach ke = 15
		...
		// check diem neu diem (height - c * 15, i ) red
                ...
                break;
            }0 <= roi.y && 0 <= roi.height && roi.y + roi.height <= m.rows) in Mat, file /home/ubuntu/opencv-3.2.0/modules/core/src/matrix.cpp, line 522
terminate called after throwing an instance of 'cv::Exception'
  what():  /home/ubuntu/opencv-3.2.0/modules/core/src/matrix.cpp:522: error: (-215) 0 <= roi.x && 0 <= roi.width && roi.x + roi.width <= m.cols && 0 <= roi.y && 0 <= roi.height && roi.y + roi.height <=
        }
        if(xLane==-1)
        {
            break;
   ...
   // Mat temp;
    ...
    /*Point target;
    if(bamlephai)
    for(int i=laneImg.cols/2;i<laneImg.cols/2+50;i++){
        if(laneImg.at<uchar>(200,i)==255){
            target=Point(i-40,200);
        }
    }
    else
    ...
    int xLaneL=-1,xLaneR=-1;
		...
		Mat tempMat;
		cvtColor(laneImg,tempMat,COLOR_GRAY2BGR);
        writer.write(tempMat);
...
void DetectSign(Mat &src)
{
    ...
    Mat hsv, gray;
    Rect roiDetect = Rect(100, 25, 150, 60);
    ...
    for (int i = 0; i < contours.size(); i++)
    {
         Rect r = boundingRect(contours[i]);
            if (abs((r.width * 1.0 / r.height) - 1) < 0.1&&r.width-6>15&&r.width-6<40)
            {
                r.x += roiDetect.x + 6;
                r.y += roiDetect.y + 6;
                r.width -= 6 * 2;
